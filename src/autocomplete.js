...
angular.module('google.places', [])
              ...
              autocompleteService.getPlacePredictions(request, function (predictions, status) {
                $scope.$apply(function () {
                  ...
                  if ($scope.predictions.length > 3) {
                    $scope.predictions.length = 3;  // trim predictions down to size
                  }
                });
              });
  ...
  .directive('gPlacesAutocompleteDrawer', ['$window', '$document', function ($window, $document) {
    var TEMPLATE = [
      '<div class="pac-container" ng-if="isOpen()" ng-style="{width: position.width+\'px\'}" style="display: block;" role="listbox" aria-hidden="{{!isOpen()}}">',
